...
parser.add_argument("-px", "--proxy", help="Set the proxy address")
parser.add_argument(
    "-g", "--get", help="Get information (use either with data_id, data_type or none)", action="store_true")
args = parser.parse_args()

#Check if there a proxy
if not args.proxy:
    exportURL = args.server
else:
    exportURL = args.proxy
# Create manage user api
data = wings.ManageData(args.server, exportURL, args.userid, args.domain)
